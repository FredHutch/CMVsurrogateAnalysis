...
bPCE <- function(dataf, iter, seed=20181211){
  ...
  bPCElist <- lapply(1:iter, function(j){
    ...
    tryCatch({
      ...
      write(paste("ERROR: Bootstrapped dataset was dropped as it had no records upon subsetting.", e$message), "logfile.csv", append= TRUE)
      #cat("ERROR: Bootstrapped dataset was dropped as it had no records upon subsetting.  \n")
      })
...
pce <- function (dataf) {
  ...
  fit <- glm(DIS56 ~ VL.1 + GCV + AGVH + CMV.D, data=dat, family="binomial")
  ...
  pY <- function(dat, fit, sz, gcv, agvh, cmv.d){
    ...
    newdata <- data.frame(VL.1=datXV$VL.1, GCV=gcv, AGVH=agvh, CMV.D=cmv.d)
...
# To run the code
bPCE(dataf, iter=11000, seed=20181211)
